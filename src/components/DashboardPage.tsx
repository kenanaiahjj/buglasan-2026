...
/* `announcements` and `candidates` are still read straight off the bundle.
   Neither is part of the ContentApi contract: `candidates` is the pre-arena
   legacy roster this screen alone still uses, and announcements have no
   endpoint. Both are listed in docs/API-INTEGRATION.md as unmigrated. */
import { announcements, candidates } from '../data/pageant';
...
function MechanicsPanel() {
  ...
  const window = festival.votingWindow?.replace(' — ', ' to ');

  return <div className="panel info-panel"><span className="eyebrow">Voting mechanics</span><h2>One account.<br /><em>One vote per day.</em></h2><p>Every registered supporter gets one vote each day during the online voting period. Choose carefully, confirm your selection, and return tomorrow to keep supporting your candidate.</p><div className="info-list">{window !== undefined && <span><Icon name="check" size={16} /> Vote from {window}</span>}<span><Icon name="check" size={16} /> Use one email or mobile account</span><span><Icon name="check" size={16} /> Votes close at 11:59 PM each day</span></div></div>;
}

function FaqPanel() {
  const { festival } = useContent();
  const faqs = [['How many votes do I get?', 'One vote per account per day while voting is open.'], ['Can I change my vote?', 'Review your candidate before you confirm. Confirmed votes cannot be changed.'], ['When does voting close?', `Online voting ends on ${festival.votingDeadline}.`]];
...
export function DashboardPage({ state, dispatch }: { state: VoterState; dispatch: Dispatch<VoterAction> }) {
  const { festival } = useContent();
  /* Half a minute, matching CountdownCard in the rail — the two are on screen
     together and disagreeing by a minute looks like a bug. */
  const now = useNow(30_000);
  const closesAt = festival.votingClosesAt === undefined ? Number.NaN : Date.parse(festival.votingClosesAt);
  const endsIn = countdownFrom(closesAt, now);
  const endsInValue = Number.isFinite(closesAt)
    ? [endsIn.days, endsIn.hours, endsIn.minutes].map((part) => String(part).padStart(2, '0')).join(' · ')
    : '—';
      ...
      <div className="dashboard-shell">
        ...
        <section className="dashboard-main">
          ...
          <div className="stat-grid">
            {/* Optional on FestivalSummary because it is derivable from the tallies.
                An em dash beats a confident zero when the server omits it. */}
            <StatCard label="Total votes" value={festival.totalVotes?.toLocaleString() ?? '—'} detail="across the festival" />
            <StatCard label="Candidates" value={String(candidates.length).padStart(2, '0')} detail="hometowns, one crown" />
            {/* Was the literal string "06 · 12 · 45", hardcoded, on every day of the
                festival. It counts now. */}
            <StatCard label="Voting ends" value={endsInValue} detail="days · hrs · mins" accent="gold" />
