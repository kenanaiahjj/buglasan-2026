...
function RankingList({ state }: { state: VoterState }) {
  ...
  return <div className="ranking-list">{ranked.map((candidate, index) => <div className="ranking-row" key={candidate.id}><span className="ranking-row__rank">{String(index + 1).padStart(2, '0')}</span><span className={`ranking-row__avatar ranking-row__avatar--${candidate.accent}`}>{candidate.initials}</span><span className="ranking-row__name"><strong>{candidate.name}</strong><small>{candidate.location}</small></span><strong className="ranking-row__votes">{state.votesByCandidate[candidate.id].toLocaleString()}</strong></div>)}</div>;
}
