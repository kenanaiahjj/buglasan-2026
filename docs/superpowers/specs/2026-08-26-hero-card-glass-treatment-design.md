...
Make the four program cards beneath the Buglasan hero read as a single black-and-gray plaque family with a subtle gold light source, then reserve each program's color for interaction.
...
- Use a mostly opaque charcoal-to-black plaque surface with a thin warm-gold edge and a low-opacity gold wash from the top edge in the default state.
- Add a contained semicircular logo chamber inside each card. Keep the logo grayscale and slightly subdued at rest, then restore its authored color on hover or keyboard focus.
- Center the program name as a large, balanced stack. Remove the hero-card subheader so the logo and title carry the card, while keeping the action available as a quiet affordance.
...
- All four hero cards use the centered logo-plaque layout and are visibly black/gray at rest with only a restrained gold tint; no chamber extends into the hero content above the card.
- The hero card surface does not create a distracting transparent double-container effect.
- Hero cards omit their subheader/tagline copy.
