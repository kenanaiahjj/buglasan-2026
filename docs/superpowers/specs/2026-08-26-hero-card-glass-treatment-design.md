...
- Use a near-black translucent glass surface with strong backdrop blur, a thin warm-gold edge, and a low-opacity gold wash from the top edge in the default state.
- Keep each program's existing accent color out of the default surface. On hover or keyboard focus, introduce that color through the top light leak, inner ray, perimeter outline, border, and restrained glow.
...
- All four hero cards are visibly black/glassy at rest with only a restrained gold tint.
- Hover and keyboard focus reveal the current program color through the card's encapsulating outline and glow.
